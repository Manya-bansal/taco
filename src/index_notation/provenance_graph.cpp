...
bool ProvenanceGraph::isConsumer(IndexVar indexVar) const{
...
bool ProvenanceGraph::isRecoverable(taco::IndexVar indexVar, std::set<IndexVar> defined) const {
  ...
  bool precompute_recover = true;
  bool non_precompute_recover = true;
  bool child_precompute = false; 
  ...
  for (auto& child: childrenRelMap.at(indexVar)) {
    ...
    if (!recoverable){
      if (child.second.getRelType() == PRECOMPUTE) precompute_recover = false;
      else non_precompute_recover = false; 
    }
...
bool ProvenanceGraph::isRecoverableStrict(taco::IndexVar indexVar, std::set<taco::IndexVar> defined) const {
...
std::vector<IndexVar> ProvenanceGraph::newlyRecoverableParents(taco::IndexVar indexVar,
  ...
  for (const IndexVar& parent : getParents(indexVar)) {
    ...
    if (!isRecoverableStrict(parent, previouslyDefined) && isRecoverableStrict(parent, defined)) {
